...
```{r}
ggplot(subset(CO2, Type == "Quebec"), aes(x=Treatment, y=uptake, fill = Treatment)) +
  geom_boxplot(show.legend = FALSE) +
  ggtitle("CO2 uptake rates of Quebec plants depending on treatment") +
  ylab("CO2 uptake rate (umol/m² sec)")
```
...
```{r}
ggplot(subset(CO2, Type == "Mississippi"), aes(x=Treatment, y=uptake, fill= Treatment)) +
  geom_boxplot(show.legend = FALSE) +
  ggtitle("CO2 uptake rates of Mississippi plants depending on treatment") +
  ylab("CO2 uptake rate (umol/m² sec)")
```
...
ggplot(CO2, aes(x=Type, y=uptake, fill=Treatment)) + geom_boxplot() +
  ggtitle("CO2 uptake rates of plants depending on origin and treatment") +
  xlab("Origin") + ylab("CO2 uptake rate (umol/m² sec)")
```
...
Looking on p-value for treatment, which is 3.107ee-3, we see that it is smaller that 0.05.
...
Looking on p-value for type, which is 4.451e-9, we see that it is smaller that 0.05.
...
```{r}
tb <- table(CO2$Type, CO2$Treatment)
chisq.test(tb)
```
...
```{r}
Treatment_fit = lm(CO2$uptake~CO2$Treatment)
anova(Treatment_fit)

Type_fit = lm(CO2$uptake~CO2$Type)
anova(Type_fit)
